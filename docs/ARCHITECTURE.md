# CubeKorean 架构

## 当前技术边界

- 类型：Web前端应用；
- 运行：React 19 + TypeScript + Vinext/Vite；
- 部署：Cloudflare Worker兼容输出，由GitHub仓库触发Cloudflare部署；
- 发音：2000词均使用随站点发布的预生成MP3并按 `1.1×` 播放，资源失败时回退浏览器Web Speech；
- 数据：十大生活主题共100关×20词位于 `app/data/lessons/`，由 `course.ts` 建立跨关卡索引；迁移词条保留稳定ID；
- 持久化：`app/features/progress/local-progress.ts` 通过版本化 `localStorage` 保存关卡进度、掌握度、复习时间与逐词错误记录；
- 后端和数据库：MVP不启用；未保留D1/Drizzle示例与依赖，需要服务端状态时先新增架构决策。

## 目标模块边界

```text
页面与关卡组件
  ↓
学习会话（轮次、题目、结果、错词）
  ↓
拼写领域规则（音节拆解、前缀判断、错误定位）
  ├── 课程数据（大关卡/小关卡/词汇）
  ├── 本机进度仓储（localStorage）
  └── 发音适配器（静态MP3优先，SpeechSynthesis备用）
```

当前已落地拼写判断、页面键盘音节组合、学习会话、课程校验、本机进度、语音适配器和10×10×20课程数据模块；页面组件负责将这些模块编排为交互流程。

学习会话支持默认“看词→听写”、可选“直接听写”，以及已通关关卡的指定五词组重练。关卡地图直接开练，模式和分组在练习页调整；仅切换模式时重开当前组并保留此前各组的统计，改变重练范围才新建范围会话。启动阶段和组索引随本机检查点恢复；单组结算只统计该组词汇，不重写整关成绩或触发整关 Coffee Tip，听写错误仍即时进入错词本。

每日活动继续使用版本化本机存储：每个词首次输入或主动看答案时记录当天的词ID，今日词数按词ID去重；整关/专项复习结果只增加完成轮次和正确率。旧活动记录没有逐词ID，保留其原有词数作为迁移基数，无法精确回填升级前未完成练习的词。

## 建议目录

```text
app/
├── components/       # 魔方、练习卡、键盘、结果组件
├── data/lessons/     # 版本化课程数据
├── features/lessons/ # 关卡选择和解锁
├── features/spelling/# 韩语拆解与拼写判断纯函数
├── features/progress/# 学习状态与本机存储
├── features/speech/  # 韩语TTS能力检测与安全降级
├── page.tsx
└── globals.css
tests/                # 核心领域和流程测试
```

不创建通用模板中的 `src/`，避免与现有 `app/` 入口形成双重结构。

## 外部依赖和回退

| 依赖 | 用途 | 本地是否必须 | 不可用表现 |
|---|---|---|---|
| 预生成静态MP3 | 稳定韩语发音 | 否 | 回退Web Speech；仍失败则提示但继续拼写 |
| Web Speech API | 静态音频失败时备用 | 否 | 显示不可用提示，拼写功能继续工作 |
| localStorage | MVP进度 | 否 | 无法跨刷新保留进度，应显示可恢复提示 |
| Cloudflare Workers | 线上发布 | 否 | 不影响本地开发 |

## 架构原则

- MVP保持单体前端，不提前引入服务端；
- 拼写判断必须是可独立测试的纯函数；
- 课程内容与UI解耦；
- 复习间隔按学习中1天、熟悉3天、连续高正确率掌握7天计算；
- 只有多设备同步或内容运营需求成立后才评估数据库和后台。
